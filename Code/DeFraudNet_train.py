...
import sys
sys.path.append('/content/drive/My Drive/DeFraudNet_dataloader')

import sys
sys.path.append('/content/drive/MyDrive/DeFraudNet_model')

import sys
sys.path.append('/content/drive/MyDrive/DeFraudNet_train')
...
def main():
    global args, best_prec1, best_ace1, writer

    # Argument parser
    parser = argparse.ArgumentParser(description="Precision Demo for Google Colab")
    ...
    args = parser.parse_args([])  # Use [] for Colab compatibility
...
def validate_model():
    global best_prec1, writer

    # Example: Update precision (replace with actual validation logic)
    precision = compute_precision()  # Replace with actual computation
    print(f"Current precision: {precision}")

    # Log precision to TensorBoard
    if writer:
        writer.add_scalar("Validation/Precision", precision)

    # Update best precision if the current precision is better
    if precision > best_prec1:
        best_prec1 = precision
        print(f"New best precision: {best_prec1}")

def compute_precision():
    # Placeholder for actual precision computation
    # Replace this with the logic to compute precision
    return 0.85  # Example precision value
...
parser = argparse.ArgumentParser(description="Normalization and Augmentation Example")
parser.add_argument("--augment", action="store_true", help="Apply data augmentation")

# Simulating argument parsing in Colab
args = parser.parse_args(["--augment"])  # Pass "--augment" to simulate enabling augmentation

# Define normalization
normalize = transforms.Normalize(
    mean=[x / 255.0 for x in [114.3, 135.0, 113.9]],
...
if args.augment:
    ...
    # Add augmentation logic here, e.g., transforms.Compose([...])
else:
...
# Define dataset
transform_train = None  # Replace with your actual transform logic
transform_train_p = None  # Replace with your actual patch transform logic

train_dataset = data.fingerprint_data('/content/drive/MyDrive/datasets/',path, transform=None, patch_transform=None)
...
# Create DataLoaders
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=32, sampler=train_sampler, **kwargs)
print("Length of the Batchwise Training set:", len(train_loader))

val_loader = torch.utils.data.DataLoader(train_dataset, batch_size=32, sampler=valid_sampler, **kwargs)
...
def ACE(output, target, topk=(1,)):
    """Computes the ACE for the specified values of k"""
    maxk = max(topk)
    batch_size = target.size(0)
    _, pred = output.topk(maxk, 1, True, True)
    pred = pred.t()
